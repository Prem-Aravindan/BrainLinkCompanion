...
# Signal processing constants from mother code
FS = 256
WINDOW_SIZE = 256 
OVERLAP_SIZE = 128
EEG_BANDS = {
...
def onRaw(raw):
    ...
    # Debug: Print occasionally to see if data is coming in
    if len(live_data_buffer) % 100 == 0:
        print(f"Raw data buffer size: {len(live_data_buffer)}, Latest value: {raw}")
...
class BrainLinkAnalyzerWindow(QMainWindow):
    ...
    def setup_timers(self):
        """Setup update timers"""
        # Live plot update timer
        self.live_timer = QTimer(self)
        self.live_timer.timeout.connect(self.update_live_plot)
        self.live_timer.start(500)  # Update every 500ms
    ...
    def update_live_plot(self):
        """Update live EEG plot using mother code processing"""
        global live_data_buffer
        
        # Update status with buffer size
        if hasattr(self, 'status_label'):
            self.status_label.setText(f"Buffer size: {len(live_data_buffer)} samples")
        
        if len(live_data_buffer) < 10:
            return
        
        try:
            # Process data exactly like mother code
            data = np.array(live_data_buffer[-256:])  # Use last 256 samples
            
            # Debug: Check if we have data
            if len(data) == 0:
                return
                
            # Downsample from 512Hz to 256Hz if needed
            if len(data) > 2:
                try:
                    data = decimate(data, 2)
                except:
                    pass  # Skip decimation if it fails
            
            fs = 256
            
            # Apply filters like mother code
            if len(data) > 10:
                try:
                ...
                # Update plot with both x and y data
                self.live_curve.setData(x_data, filtered)
                
                # Add data to feature engine
                if len(filtered) > 0:
                    self.feature_engine.add_data(filtered[-1])
                    
        except Exception as e:
            print(f"Error in update_live_plot: {e}")
            # Fallback: plot raw data if processing fails
            if len(live_data_buffer) > 0:
                raw_data = np.array(live_data_buffer[-100:])  # Last 100 samples
                x_data = np.arange(len(raw_data))
                self.live_curve.setData(x_data, raw_data)
